...
import ecma.ai.hrapp.component.checkPositionPayload.CheckPosition;
...
public class Checker {
    ...
    public CheckPosition checkPosition() {
        ...
        if (optionalUser.isPresent()) {
            String position = "";
            String positions = optionalUser.get().getPosition();
            if (positions.contains("director")) position+=positions;
            if (positions.contains("manager")) position+="manager";
            return new CheckPosition(optionalUser.get(), position);
        }
        return null;
    }
